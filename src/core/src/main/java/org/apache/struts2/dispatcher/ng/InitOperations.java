...
public class InitOperations {
    ...
    public void initLogging( HostConfig filterConfig ) {
        //1、-----------获取web.xml文件的配置的init-param中的loggerFactory属性值
        String factoryName = filterConfig.getInitParameter("loggerFactory");

        //2、-----------若web.xml配置了loggerFactory属性值,则加载该类并获取其实例对象，并设置给struts自己内部的日志LoggerFactory类的factory属性
        if (factoryName != null) {
            try {
    ...
    public Dispatcher initDispatcher( HostConfig filterConfig ) {
        //1、--------创建Dispatcher（即初始化Dispatcher对象的属性）
        Dispatcher dispatcher = createDispatcher(filterConfig);

        //2、--------对创建的Dispatcher实例对象进行初始化
        dispatcher.init();
    ...
    private Dispatcher createDispatcher( HostConfig filterConfig ) {
        System.out.println("======================创建Dispatcher对象，createDispatcher()....");
        //1、------------获取web.xml文件中配置的init-param属性名称和值，并封装进Map对象中
        Map<String, String> params = new HashMap<String, String>();
        for ( Iterator e = filterConfig.getInitParameterNames(); e.hasNext(); ) {
            String name = (String) e.next();
            String value = filterConfig.getInitParameter(name);
            params.put(name, value);
            System.out.println("======================创建Dispatcher对象时，web.xml文件中配置的init-param属性,name:" +name+"，value："+value);
        }

        //2、------------创建一个给定的ServletContext和设置初始化参数的Dispatcher实例（即用ServletContext和设置的初始化参数赋值给Dispatcher的servletContext和initParams属性）
