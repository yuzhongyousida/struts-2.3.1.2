...
public class Dispatcher {
    ...
    /**
     * 初始化各种形式加载器,保存到ConfigurationManager类的containerProviders Map集合属性中 没有真正执行加载 解析逻辑
     * 加载org/apache/struts2/default.properties属性文件  里面定义了一系列struts常量定义
     */
    private void init_DefaultProperties() {
        //default.properties 属性文件加载器
    ...
    private void init_TraditionalXmlConfigurations() {
        ...
        //2、-------------将这些文件的配置字符串分隔之后，用这些文件名生成对应的XmlConfigurationProvider对象，并封装进configurationManager对象的containerProviders List属性中
        String[] files = configPaths.split("\\s*[,]\\s*");
        for (String file : files) {
            System.out.println("==================获取web.xml的init-param配置中的config配置，fileName:"+file);
            if (file.endsWith(".xml")) {
                if ("xwork.xml".equals(file)) {
                    configurationManager.addContainerProvider(createXmlConfigurationProvider(file, false));
                } else {
                    //struts xml加载器 XmlConfigurationProvider
    ...
    /**
     * 初始化配置的自定义加载器  保存到configurationManager的containerProviders List属性中
     */
    private void init_CustomConfigurationProviders() {
        //1、------------获取web.xml文件配置的init-param中的configProviders属性值
        String configProvs = initParams.get("configProviders");

        //2、------------根据名称，加载这些配置的加载器类，并获取其实例对象，并封装在configurationManager的containerProviders List属性中
        if (configProvs != null) {
     ...
     */
    public void init() {
        System.out.println("==================Dispatcher.init()方法执行。。。");

        //1、--------------初始化装载ConfigurationManager实例的属性（以“struts”字符串做入参）
    	if (configurationManager == null) {
    		configurationManager = createConfigurationManager(BeanSelectionProvider.DEFAULT_BEAN_NAME);
    	}

        try {
            //2、--------------初始化各种形式加载器,保存到ConfigurationManager类的containerProviders Map集合属性中 没有真正执行加载 解析逻辑
            init_DefaultProperties(); // [1]  org/apache/struts2/default.properties属性文件  里面定义了一系列struts常量定义

            //3、--------------初始化struts的xml文件的配置provider信息（获取web.xml的init-param配置中的config名称对应的struts相关配置文件（struts-default.xml,struts-plugin.xml,struts.xml等struts的配置文件路径））
            init_TraditionalXmlConfigurations(); // [2]

            //4、--------------初始化strutus的properties文件的配置provider信息
            init_LegacyStrutsProperties(); // [3]

            //5、--------------初始化配置的加载器  保存到configurationManager的containerProviders List属性中
            init_CustomConfigurationProviders(); // [5]

            //6、--------------web.xml配置的init-param参数 加载器  最终会保存到Container容器中
            init_FilterInitParameters() ; // [6]

            //7、--------------根据我们在struts.xml定义的常量 选择插件类或者初始化一些属性。比如：集成spring 会用到org.apache.struts2.spring.StrutsSpringObjectFactory；
            init_AliasStandardObjects() ; // [7]

            //8、--------------执行加载器:创建容器,解析xml,真正执行加载器方法
            Container container = init_PreloadConfiguration();

            //9、--------------执行当前Dispatcher对象 依赖关系注入
